...
// ==========================================
// 7. SEND OTP FOR HOSTEL REMOVAL CONFIRMATION
// ==========================================
const sendRemoveHostelOtp = async (req, res, next) => {
    const { hostelId } = req.params;
    ...
    try {
        ...
        await generateAndSendOTP(
            adminEmail,
            'Mess Mate - Confirm Hostel Removal',
            `<p>Hello Admin,</p>
            <p>You requested to remove the <strong>Hostel ${escapeHtml(String(hostelId))}</strong>.</p>
            <p><strong>Your confirmation OTP is: {{OTP}}</strong></p>
            <p>This code is valid for the next 5 minutes. If you did not request this, please secure your account and contact support immediately.</p>
            <p>Best regards,<br/>The Mess Mate Team<br/>National Institute of Technology, Kurukshetra</p>`
        );
...
const removeHostel = async (req, res, next) => {
    ...
    try {
        ...
        const hostelObjectId = hostel._id;

        // Need student ids first to clean up their purchases
        const students = await User.find({ hostel: hostelObjectId, role: 'student' }).select('_id');
        const studentIds = students.map(s => s._id);

        // cascading delete
        await Promise.all([
            User.deleteMany({ hostel: hostelObjectId }), // removes students + accountant
            studentIds.length ? Purchase.deleteMany({ student: { $in: studentIds } }) : Promise.resolve(),
            Rating.deleteMany({ hostel: hostelObjectId }),
            Item.deleteMany({ hostel: hostelObjectId }),
            WeeklyMenu.deleteMany({ hostel: hostelObjectId }),
            DailyMenu.deleteMany({ hostel: hostelObjectId }),
            ReviewAnalysis.deleteOne({ hostel: hostelObjectId }),
        ]);

        // delte hostel
        await Hostel.findByIdAndDelete(hostelObjectId);

        // delte cache
        await Promise.all([
            invalidateKeys(keys.hostelsPublicList(), keys.hostelsAdminList()),
            invalidatePattern(keys.menuAllPattern(hostelObjectId.toString())),
            invalidatePattern(keys.extrasAllPattern(hostelObjectId.toString())),
        ]);

        res.json({ message: 'Hostel and all associated data removed successfully.', hostelId: hostelId, studentsRemoved: studentIds.length});
    } catch (error) {
        next(error);
    }
};
